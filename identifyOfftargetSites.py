...
class chromosomePosition():
    ...
    def addPositionBarcode(self, chromosome, position, strand, barcode, primer, count):
        ...
        if position not in self.chromosome_barcode_dict[chromosome]:
            ...
            self.chromosome_barcode_dict[chromosome][position]['+primer1E_total'] = 0
            self.chromosome_barcode_dict[chromosome][position]['+primer2E_total'] = 0
    ...
    def SummarizeBarcodePositions(self):
        self.barcode_position_summary = [[chromosome, position,
                                          ...
                                          len(self.chromosome_barcode_dict[chromosome][position]['-primer1']),
                                          len(self.chromosome_barcode_dict[chromosome][position]['-primer2']),
                                          len(self.chromosome_barcode_dict[chromosome][position]['+primer1E']),
                                          len(self.chromosome_barcode_dict[chromosome][position]['+primer2E']),
                                          len(self.chromosome_barcode_dict[chromosome][position]['-primer1E']),
                                          len(self.chromosome_barcode_dict[chromosome][position]['-primer2E']),
                                          ]
    ...
    # Summarizes the chromosome, positions within a 10 bp window, zhike20bp
    def SummarizeBarcodeIndex(self):
        ...
        for chromosome, position, barcode_plus_count, barcode_minus_count, total_plus_count, total_minus_count, plus_primer1_count, plus_primer2_count,\
                minus_primer1_count, minus_primer2_count,plus_primer1E_count, plus_primer2E_count,minus_primer1E_count, minus_primer2E_count in self.barcode_position_summary:
#           if chromosome != last_chromosome or abs(position - last_position) > 10:
            if chromosome != last_chromosome or abs(position - last_position) > 20: #######zhike
            ...
            self.index_stack[window_index].append([chromosome, int(position),
                                                   ...
                                                   int(plus_primer1E_count), int(plus_primer2E_count),
                                                   int(minus_primer1E_count), int(minus_primer2E_count)
                                                   ])
        for index in self.index_stack:
            ...
            chromosome_list, position_list, \
                ...
                plus_primer1E_list, plus_primer2E_list, minus_primer1E_list, minus_primer2E_list\
                = zip(*sorted_list)
            ...
            plus_primer1E = sum(plus_primer1E_list)
            plus_primer2E = sum(plus_primer2E_list)
            minus_primer1E = sum(minus_primer1E_list)
            minus_primer2E = sum(minus_primer2E_list)
            ...
            primer1E = plus_primer1E + minus_primer1E
            primer2E = plus_primer2E + minus_primer2E
            primerE_geometric_mean = (primer1E * primer2E) ** 0.5
#           primer1 = plus_primer1 + minus_primer1
#           primer2 = plus_primer2 + minus_primer2
            primer1 = plus_primer1 + minus_primer1 + primer1E
            primer2 = plus_primer2 + minus_primer2 + primer2E
...
#           offtarget_sequence = self.getSequence(self.genome, most_frequent_chromosome, most_frequent_position - 25, most_frequent_position + 25)
            offtarget_sequence = self.getSequence(self.genome, most_frequent_chromosome, most_frequent_position - 35, most_frequent_position + 35) #zhike
            ...
            summary_list = [str(x) for x in [index, most_frequent_chromosome, most_frequent_position, offtarget_sequence,                        # pick most frequently occurring chromosome and position
                                             ...
                                             primer1, primer2, primer_geometric_mean, primer1E, primer2E, primerE_geometric_mean, position_std]]

            if (barcode_geometric_mean > 0 or primer_geometric_mean > 0 or primerE_geometric_mean > 0):
...
#                primer = assignPrimerstoReads(read_sequence, sam_flag)  ###########zhike
    ...
    with open(outfile, 'w') as f:
        ...
        f.write('\t'.join(['#BED Chromosome', 'BED Min.Position',
                           ...
                           '-.total', 'total.sum', 'total.geometric_mean', 'primer1.mi', 'primer2.mi', 'primer.geometric_mean', 'primer1E.mi', 'primer2E.mi', 'primerE.geometric_mean',
...
#                    target_end_absolute = target_end_relative + int(row[2]) - 25
                    target_start_absolute = target_start_relative + int(row[2]) - 35 #zhike
                    target_end_absolute = target_end_relative + int(row[2]) - 35 #zhike
                elif strand == "-":
#                    target_start_absolute = int(row[2]) + 25 - target_end_relative
#                    target_end_absolute = int(row[2]) + 25 - target_start_relative
                    target_start_absolute = int(row[2]) + 35 - target_end_relative #zhike
...
#       readstart = reverseComplement(read_sequence[-20:])
        readstart = reverseComplement(read_sequence[-22:]) #modified by zhike
    else:
#       readstart = read_sequence[:20]
        readstart = read_sequence[:22] #modified by zhike
#    if readstart == "TTGAGTTGTCATATGTTAAT":
    if re.match('^ACCAATCCACCAACCTC',readstart,re.IGNORECASE): #modified by zhike
        return "primer1E"
    elif re.match('^ACCAATCCACCAACC',readstart,re.IGNORECASE): #modified by zhike
        return "primer1"
#    elif readstart == "ACATATGACAACTCAATTAA":
    elif re.match('^TCCACCCCGCACCAATCCACCA',readstart,re.IGNORECASE): #modified by zhike
        return "primer1E"
    elif re.match('^TCCACCCCGCACCAATCCAC',readstart,re.IGNORECASE): #modified by zhike
        return "primer1"
    elif re.match('^CCCCCTCCACCCCGCACCAATC',readstart,re.IGNORECASE): #modified by zhike
        return "primer1E"
    elif re.match('^CCCCCTCCACCCCGCACCAA',readstart,re.IGNORECASE): #modified by zhike
        return "primer1"
    elif re.match('GTTGGTGGATTGGT',readstart,re.IGNORECASE): #modified by zhike
        return "primer2E"
    elif re.match('GTTGGTGGATT',readstart,re.IGNORECASE): #modified by zhike
        return "primer2"
    else:
        return "nomatch"
...
#       return molecular_index, int(count)
        molecular_index, count, p = m.group(1), m.group(2), m.group(3)
        if p == '1':
            primer = "primer1"
        elif p == "2":
            primer = "primer2"
            ...
            primer = None
        return molecular_index, int(count), primer, p
    else:
        # print(read_name)
#       return None, None
